...
`r shiny::icon("github")` https://github.com/derekmichaelwright/AGILE_LDP_Nepal
...
<font size="5"> `r shiny::icon("save")` [data_countries.csv](data_countries.csv) </font>

<font size="5"> `r shiny::icon("save")` [data_pca_results.csv](data_pca_results.csv) </font>

<font size="5"> `r shiny::icon("save")` [model_t+p_coefs.csv](model_t+p_coefs.csv) </font>
...
mp <- ggplot(dd) +
  geom_hline(data = myThresholds, aes(yintercept = Cutoff), alpha = 0.7) +
  geom_quasirandom(aes(x = Origin, y = Value, alpha = Selection, group = Origin,
...
<font size="5"> `r shiny::icon("save")` [model_nepal.csv](model_nepal.csv) </font>
